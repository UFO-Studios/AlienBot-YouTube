...
const setGToken = async (AccessToken, RefreshToken, Scope, TokenType, ExpiryDate, UUID) => {
    if (!connected || !db) {
        await connectToDB()
    }

    const dataToDB = GTokenModule({ AccessToken, RefreshToken, Scope, TokenType, ExpiryDate, UUID });
